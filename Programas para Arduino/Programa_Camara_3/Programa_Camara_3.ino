/****************************************************************************************************************************
* Proyecto : Tablero para monitoreo de Humedad y Temperatura ambiente con sensor DHT22
...
*
* Descripción: Leer humedad y temperatura ambiente con el sensor DHT22, almacenarlo en variables globales 
*              y luego mostrarlo en Displays de 7 segmentos.
*              Esperar a través del puerto Serial la trama de datos del tablero más cercano.
*              Incorporar a la trama recibida los valores de humedad y temperatura y luego enviar al 
*              siguiente tablero a través del MAX485.
*
* Conexión:
* (MÓDULO HC-06)    (ARDUINO Nano)            
*     RX        ->   No se conecta             
*     TX        ->   Pin 10                       
*     VCC       ->   VCC                          
*     GND       ->   GND  
...
*     VCC       ->   VCC
*
*  Cables de los Displays (Puerta del tablero):
*     DATA      ->   Pin 2
*     CLK       ->   Pin 3
*     STB 0     ->   Pin 4
*     STB 1     ->   Pin 5
...
void recibe_carga_envia_trama()
{
   while( Serial.available())
   {
     ...
     } else if (ch == ',') // si detecta coma, mueve al siguiente campo
     {
           ...
           save_values_2[20] = buffer1;            
            
           char buffer2[7];
           dtostrf(HUMEDAD, 7, 2, buffer2);
           save_values_2[21] = buffer2;            
